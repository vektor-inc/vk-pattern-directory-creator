 ...
 */
function vkpdc_content_single_smaveksive( $content ) {
	if ( is_singular() ) {
		...
		if ( 'vk-patterns' === $post->post_type && empty( $_GET['view'] ) ) {
			...
			$select_button = vkpdc_get_size_selector_smaveksive();

			// Iframe を適用したコンテンツを取得.
			$iframe_content = '<div class="vkpdc_iframe-outer vkpdc_iframe-outer--single">' . vkpdc_get_iframe_content_smaveksive( $post_id, 'single' ) . '</div>';
...
add_filter( 'the_content', 'vkpdc_content_single_smaveksive', 0 );
