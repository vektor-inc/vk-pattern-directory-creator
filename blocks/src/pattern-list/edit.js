...
export default function PostListEdit( props ) {
	...
	// リンクを無効にする関数
	useEffect(() => {
		const targetNode = document.querySelector('.edit-post-visual-editor iframe');
		const iframeDocument = targetNode?.contentDocument || document;
	
		const observer = new MutationObserver(() => {
			const links = iframeDocument.querySelectorAll(
				'.vkpdc_post a, .vkpdc_pagination a'
			);
			links.forEach((link) => {
